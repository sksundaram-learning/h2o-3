...
#' as merge() in base R.  However, we do not support all=TRUE, all.x=TRUE and all.y=TRUE.  The default method is radix
#' in addition to the hash method.  If there are duplicated
#' rows in your rite frame, they will not be included if you use the hash method.  Since it is rare to perform merge
#' with duplicated rows an the right frames, this should be okay.  On the other hand, radix method will return the
#' correct merge regardless of duplicated rows in the right frame.  However, it cannot merge frames containing
...
#' @param method auto, radix(default), hash
#' @examples
#' \donttest{
#' h2o.init()
